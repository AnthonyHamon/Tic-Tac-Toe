...
function render() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      tableHTML += `<td id="cell-${index}" onclick="handleCellClick(this, ${index})">${symbol}</td>`;
    }
...
function handleCellClick(clickedCell, index) {
  if (fields[index] === null && !gameOver) { // Überprüfe, ob gameOver false ist
  ...
  function getWinningCombination() {
    for (let i = 0; i < WINNING_COMBINATIONS.length; i++) {
      ...
      if (fields[a] === fields[b] && fields[b] === fields[c] && fields[a] !== null) {
        gameOver = true; // Setze gameOver auf true
...
function restartGame() {
  // Setze alle Felder im Array "fields" auf null zurück
